...
| `X-User-Permissions` | 逗号分隔的权限快照。 |
| `X-Authorization-Snapshot-Id` | Gateway 签发、可在服务端授权快照表验证的 UUID。 |
| `X-Service-Authorization` | 面向目标服务的短期服务 JWT。 |

外部请求携带的同名头必须在 Gateway 删除。服务不允许从请求参数、请求体或前端缓存推断租户、角色、权限或知识域授权；检索必须以 `X-Authorization-Snapshot-Id` 在服务端验证主体可访问的知识域。
