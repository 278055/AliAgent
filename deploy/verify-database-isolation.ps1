...
foreach ($source in $databases) {
    foreach ($target in $databases) {
        $result = docker compose -f $ComposeFile --env-file $EnvFile exec -T postgres bash -lc "PGPASSWORD='$($source.Password)' psql -h localhost -U '$($source.User)' -d '$($target.Name)' -Atc 'SELECT 1'" 2>$null
