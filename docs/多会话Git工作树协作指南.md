...
示例：P1 的三个任务分支为 `codex/p1-platform-contracts`、`codex/p1-service-skeletons`、`codex/p1-infra-frontend`；阶段集成分支为 `codex/integration-p1`。
...
# 按依赖顺序合并；P1 依次为平台/契约、服务骨架、本地环境/前端。
git merge --no-ff codex/p1-platform-contracts -m "merge(p1): 平台与契约"
git merge --no-ff codex/p1-service-skeletons -m "merge(p1): 服务骨架"
git merge --no-ff codex/p1-infra-frontend -m "merge(p1): 本地环境与前端"
```
...
```powershell
# 后端聚合测试
mvn -s D:\Java_Tools\Maven\apache-maven-3.9.6\conf\settings.xml test

# Compose 静态校验
docker compose -f deploy/docker-compose.yml config

# 前端兼容构建（依 P1-C 实际脚本调整）
Set-Location frontend
npm run build
```

还必须验证：基础设施可启动、五个 PostgreSQL 数据库及独立账号存在、`knowledge_db` 启用 `vector` 扩展、服务健康接口可用、未授权服务身份被拒绝、契约校验通过。端到端测试产生的资源必须使用 `test-` 或 `rag-test-` 前缀并清理。
...
gh pr create --base main --head codex/integration-p1 --title "feat(p1): 工程与基础设施" --body "阶段集成测试已通过，详见 PR 检查项。"
```
...
```powershell
Set-Location D:\Java\code\AliAgent
git worktree remove D:\Java\code\AliAgent-worktrees\p1-platform-contracts
git worktree remove D:\Java\code\AliAgent-worktrees\p1-service-skeletons
git worktree remove D:\Java\code\AliAgent-worktrees\p1-infra-frontend
git worktree remove D:\Java\code\AliAgent-worktrees\p1-integration
git branch -d codex/p1-platform-contracts
git branch -d codex/p1-service-skeletons
git branch -d codex/p1-infra-frontend
