...
public record TrustedConversationRequestContext(String tenantId, String subjectId, String traceId, UUID requestId) {
    ...
    public static TrustedConversationRequestContext from(HttpServletRequest request) {
        ...
        try {
            return new TrustedConversationRequestContext(required(request, "X-Tenant-Id"), required(request, "X-Subject-Id"),
                    required(request, "X-Trace-Id"), UUID.fromString(required(request, "X-Request-Id")));
